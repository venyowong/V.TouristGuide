    ...
    mounted() {
      if (common.addPageView) {
        ...
        if (user) {
          ...
          fetch(`https://vbranch.cn/talog/metric/pg/add?index=tg&page=modifyLocation&user=${user.id}`);
        }
        else {
          fetch('https://vbranch.cn/talog/metric/pg/add?index=tg&page=modifyLocation');
        }
      }
